...
-- 9. Account Deletion Security Triggers
-- Prevents users from manually setting a deletion date in the past to bypass the grace period.
CREATE OR REPLACE FUNCTION public.protect_deletion_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    -- If the date is being set (transition from NULL to NOT NULL), force it to be NOW()
...
-- 10. Default System Categories (Seed)
